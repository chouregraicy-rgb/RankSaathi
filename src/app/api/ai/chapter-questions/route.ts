  ...
  for (const model of MODELS) {
    try {
      const res = await fetch(GROQ_URL, {
        ...
        body: JSON.stringify({ model, messages: [{ role: "user", content: prompt }], max_tokens: 3000, temperature: 0.7 }),
      });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `Generate ${count} ${difficulty} difficulty MCQ questions for ${exam}.
Subject: ${subject}, Chapter: ${chapter}

Return ONLY a valid JSON array:
[{"id":1,"question":"?","options":["A","B","C","D"],"correctAnswer":0,"explanation":"why","difficulty":"${difficulty}"}]`;
